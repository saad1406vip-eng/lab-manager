...
function saveItem() {
  ...
  supply.lot = document.getElementById("itemLot").value.trim();
  supply.expiry = document.getElementById("itemExpiry").value;
  supply.openDate = document.getElementById("itemOpenDate").value;
  supply.kitDays = parseInt(document.getElementById("itemKitDays").value) || 0;
  supply.location = document.getElementById("itemLocation").value.trim();
  supply.note = document.getElementById("itemNote").value.trim();

  saveSupplies();
  closeModal();

  /* إعادة الرسم */
  renderInventoryForm(currentEdit.employeeId, currentEdit.deviceId);
  showToast("✓ تم الحفظ", "success");
}

function closeModal() {
  document.getElementById("modalOverlay").classList.remove("active");
  currentEdit = { deviceId: null, supplyIdx: null, employeeId: null };
}
...
function printInventory(empId, deviceId) {
  ...
  if (!win) {
    showToast("⚠️ اسمح بالنوافذ المنبثقة للطباعة", "warning");
    return;
  }
...
function buildPrintHTML(employee, device) {
  ...
  html += '<scr' + 'ipt>window.onload=function(){setTimeout(function(){window.print();},300);};</scr' + 'ipt>';
  html += '</body></html>';
...
function renderDevicesAdmin() {
  ...
  devices.forEach(function (device) {
    ...
    html += '<div class="device-card-icon">' + getDeviceIcon(device.id) + '</div>';
    html += '<div class="device-card-info">';
    html += '<h4 class="device-card-name">' + device.name + '</h4>';
    html += '<p class="device-card-meta">' + device.company + ' · ' + device.supplies.length + ' محلول</p>';
    html += '</div>';
    html += '</div>';
  });
...
function renderDeviceCheckboxes() {
  ...
  Object.keys(SUPPLY_DATA).forEach(function (devId) {
    ...
    html += '<span class="checkbox-icon">' + getDeviceIcon(devId) + '</span>';
...
function addNewEmployee() {
  ...
  if (!name) {
    showToast("أدخل اسم الموظف", "warning");
    return;
  }
