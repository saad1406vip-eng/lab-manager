...
function showPage(pageId) {
  try {
    ...
    if (targetPage) {
      ...
      console.log('✅ انتقل إلى صفحة:', pageId);
    } else {
      console.error('❌ الصفحة غير موجودة:', pageId);
    }
  } catch (error) {
    console.error('❌ خطأ في showPage:', error);
  }
}
...
function startInventory() {
  ...
  if (!empId || !deviceId) {
    showToast('اختر الاسم والجهاز أولاً', 'warning');
    return;
  }
  ...
  console.log('✅ بدء الجرد:', employee.name, device.name);
...
function renderInventoryForm(empId, deviceId, employee, device) {
  ...
  if (!device || !device.supplies || device.supplies.length === 0) {
    container.innerHTML = '<div class="card"><p style="color: #f44; text-align: center;">⚠️ لا توجد محاليل لهذا الجهاز</p></div>';
    return;
  }
  ...
  html += `
      ...
      <div style="margin-top: 1rem; text-align: center;">
        <button class="btn-primary" onclick="showPage('inventory'); document.getElementById('inventoryContent').innerHTML = '';">
          ↩️ العودة
        </button>
      </div>
    </div>
