/* ═══════════════════════════════════════════════════════════
   ...
   المنطق الكامل: الجرد + الإدارة + إضافة/حذف البنود
   ═══════════════════════════════════════════════════════════ */
...
var SYSTEM_DATA = {
  employees: [],
  employee_devices: {},
  custom_supplies: {}  /* البنود المضافة يدوياً */
};
...
function loadSystemData() {
  /* تحميل بيانات النظام (الموظفون + الربط + البنود المخصصة) */
  ...
  /* تحميل حالة المخزون (الكميات والتواريخ) */
  ...
  Object.keys(SYSTEM_DATA.custom_supplies).forEach(function (devId) {
    if (SUPPLY_DATA[devId]) {
      SYSTEM_DATA.custom_supplies[devId].forEach(function (item) {
        /* تأكد أن البند غير موجود مسبقاً */
...
function refreshAll() {
  ...
  populateEmployeeSelect();
  populateAdminUI();
  /* إعادة رسم الجرد إن كان مفتوحاً */
...
function renderInventoryForm(empId, deviceId) {
  ...
  Object.keys(SUPPLY_TYPES).forEach(function (type) {
    ...
    grouped[type].forEach(function (item) {
      ...
      html += '<button class="btn-delete-item" onclick="confirmDeleteItem(\'' + deviceId + '\',' + idx + ',\'' + empId + '\')" title="حذف">🗑</button>';
      html += '</div>';
    });
...
function saveItem() {
  ...
  if (currentEdit.supplyIdx === -1) {
    ...
    device.supplies[currentEdit.supplyIdx] = itemData;

    /* تحديث البنود المخصصة إن كانت موجودة */
  ...
  closeModal();
  renderInventoryForm(currentEdit.employeeId, deviceId);
}
...
function deleteItem(deviceId, supplyIdx, empId) {
  ...
  /* حذف من البنود المخصصة إن كان مضافاً */
  if (SYSTEM_DATA.custom_supplies[deviceId]) {
